...
Es bietet sich an, \emph{KnowNet} durch eine Ontologie ab zu bilden. Ontologien die auf dem OWL-Standard basieren, bieten die M\"oglichkeit an, mittels einer in den Grundz\"ugen einfachen Sprache eine Wissensdatenbank komplex verkn\"upft zu speichern und darauf Abfragen aus zu f\"uhren. OWL ist eine Erweiterung von RDF/S und ist wie dieses durch die zwei Syntaxen \emph{Turtle} und \emph{XML} definiert. Obwohl \emph{XML} im Vergleich zu \emph{Turtle} einiges mehr ''drum herum'' hat, ist es dennoch einfacher zu erstellen und auch zu pflegen denn \emph{Turtle}. Auch kann die Ontologie in dieser Syntak durch andere Programme einfacher gelesen werden und ist auch einfacher wartbar, da Syntaxfehler durch gute Editoren schneller erkennt werden.

OWL ist aktuell als OWL (Version 1) und OWL-2 definiert. Der OWL-2 Standard ist eine totale Entkoppelung von RDF/S und enth\"allt Erweiterungen die in der Ursprungsversion fehlen. Die Aktuelle Ontologie wird noch mit OWL-1 implementiert, da ich diese Syntax und deren Eigenschaften vom Studium her kenne. Auch weiss ich nicht, inwieweit die vorhandenen Libraries für OWL schon auf OWL-2 angepasst sind.
...
\begin{description}
  \item[Ontologie] OWL in Version 1, also mit RDF/S Elementen
  \item[Abfrage] SPARQL
  \item[Programmierung] C/C++ mit Qt4
  \item[Eventuell] HTML, Javascript, ...
  \item[Libraries] Wahrscheinlich ''Redland RDF Libraries'' \url{http://librdf.org/} oder ''RDFStore'' von \url{http://rdfstore.sourceforge.net/}
\end{description}
